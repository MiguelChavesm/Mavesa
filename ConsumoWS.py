...
class ProcesadorArchivos:
    def __init__(self, root):
        ...
        # Nuevo: Estado de bloqueo de configuración
        self.configuracion_bloqueada = False
        ...
        self.notebook.add(self.configuracion_tab,
                          text="Configuración", state="disabled")
        ...
        self.notebook.bind("<<NotebookTabChanged>>",
                           self.abrir_pestana_configuracion)
...
# Metodos para configuración del .ini
    ...
    def cargar_configuracion(self):
        ...
        if 'Configuracion' in config:
            self.token_url.set(self.desencriptar(
                config['Configuracion'].get('Token_url', '')))
            self.api_url.set(self.desencriptar(
                config['Configuracion'].get('api_url', '')))
            self.url_api_image.set(self.desencriptar(
                config['Configuracion'].get('url_api_image', '')))
            self.client_id.set(self.desencriptar(
                config['Configuracion'].get('client_id', '')))
            self.client_secret.set(self.desencriptar(
                config['Configuracion'].get('client_secret', '')))
            self.username.set(self.desencriptar(
                config['Configuracion'].get('username', '')))
            self.password.set(self.desencriptar(
                config['Configuracion'].get('password', '')))
            self.carpeta_archivos.set(self.desencriptar(
                config['Configuracion'].get('carpeta_archivos', '')))
            self.carpeta_imagenes.set(self.desencriptar(
                config['Configuracion'].get('carpeta_imagenes', '')))
            self.contraseña = (self.desencriptar(config['Configuracion'].get(
                'contraseña_adicional', 'MONTRA101')))  # Obtener la contraseña

    def guardar_configuracion(self):
        config = configparser.ConfigParser()
        config.read('config.ini')
        config['Configuracion']['Token_url'] = self.encriptar(
            self.token_url.get())
        config['Configuracion']['api_url'] = self.encriptar(self.api_url.get())
        config['Configuracion']['url_api_image'] = self.encriptar(
            self.url_api_image.get())
        config['Configuracion']['client_id'] = self.encriptar(
            self.client_id.get())
        config['Configuracion']['client_secret'] = self.encriptar(
            self.client_secret.get())
        config['Configuracion']['username'] = self.encriptar(
            self.username.get())
        config['Configuracion']['password'] = self.encriptar(
            self.password.get())
        config['Configuracion']['carpeta_archivos'] = self.encriptar(
            self.carpeta_archivos.get())
        config['Configuracion']['carpeta_imagenes'] = self.encriptar(
            self.carpeta_imagenes.get())
        config['Configuracion']['contraseña_adicional'] = self.encriptar(
            (self.contraseña))
...
# Pestaña de ingreso de contraseña
    ...
    def abrir_pestana_configuraciones(self):
        if not self.contraseña_verificada:
            if not hasattr(self, "dialogo_contraseña") or not self.dialogo_contraseña.winfo_exists():
                ...
                etiqueta_contraseña = ttk.Label(
                    self.dialogo_contraseña, text="Ingrese la contraseña:")
                etiqueta_contraseña.pack(pady=5)

                self.entrada_contraseña = ttk.Entry(
                    self.dialogo_contraseña, show="*")
                self.entrada_contraseña.pack(pady=5)
                self.entrada_contraseña.focus_set()
                self.entrada_contraseña.bind(
                    "<Return>", lambda event: self.verificar_contraseña())

                boton_verificar = ttk.Button(
                    self.dialogo_contraseña, text="Verificar", command=self.verificar_contraseña)
                boton_verificar.pack(pady=5)

                # Configurar una acción cuando se cierra la ventana de verificación
                self.dialogo_contraseña.protocol(
                    "WM_DELETE_WINDOW", self.restablecer_campos_configuracion)
    ...
    def restablecer_campos_configuracion(self):
        ...
        if self.dialogo_contraseña:
            ...
            # Verifica si la pestaña actual es la de configuración
            if self.notebook.index("current") == 1:
                self.notebook.select(0)  # Cambia a la pestaña de medición
                # Deshabilita la pestaña de configuración
                ...
                # Si se cierra la ventana de configuración desde la pestaña de medición
                self.configuracion_bloqueada = False
    ...
    def verificar_contraseña(self):
        ...
        if contraseña_ingresada == self.contraseña:
            # Habilita la pestaña de configuración
            self.notebook.tab(1, state="normal")
            self.dialogo_contraseña.destroy()
            # Habilitar los campos de configuración
            for child in self.configuracion_tab.winfo_children():
...
# Pestaña para cambio de contraseña
    def abrir_ventana_cambio_contraseña(self):
        ...
        ttk.Label(cambio_contraseña_window, text="Contraseña Actual:").grid(
        ...
        ttk.Label(cambio_contraseña_window, text="Nueva Contraseña:").grid(
        ...
        ttk.Button(cambio_contraseña_window, text="Guardar", command=lambda: self.guardar_nueva_contraseña(
            contraseña_actual_entry.get(), nueva_contraseña_entry.get(), cambio_contraseña_window)).grid(row=2, columnspan=2, padx=10, pady=5)

        cambio_contraseña_window.bind("<Return>", lambda event: self.guardar_nueva_contraseña(
            contraseña_actual_entry.get(), nueva_contraseña_entry.get(), cambio_contraseña_window))
...
# Creación de ventanas de interfaz
    def create_medicion_tab(self):
        ...
        self.background = ttk.Label(
            self.medicion_tab, background=self.colorbackground)
        self.background.grid(row=0, column=0, columnspan=21,
                             rowspan=2, pady=(0, 0), sticky="snew")

        self.label_imagen1 = ttk.Label(
            self.medicion_tab, image=self.logo_montra, background=self.colorbackground)
        self.label_imagen1.grid(
            row=0, column=0, columnspan=2, padx=(30, 0), pady=(10, 0))

        self.label_imagen3 = ttk.Label(
            self.medicion_tab, image=self.logo_cubiscan, background=self.colorbackground)
        self.label_imagen3.grid(
            row=1, column=0, columnspan=2, padx=(30, 0), pady=(0, 10))

        self.label_imagen2 = ttk.Label(
            self.medicion_tab, image=self.logo_mavesa, background=self.colorbackground)
        self.label_imagen2.grid(
            row=0, column=19, rowspan=2, columnspan=2, padx=(0, 0), sticky="w")
        ...
        self.boton_iniciar = tk.Button(self.medicion_tab, text="Iniciar", command=self.iniciar_proceso,
                                       relief="groove", padx=10, pady=5, borderwidth=3, width=10, background="lightgrey")
        self.boton_iniciar.grid(
            row=0, column=2, columnspan=3, padx=(85, 55),  pady=10, stick="w")
        ...
        self.boton_detener.grid(
            row=1, column=2, columnspan=2, sticky="e", padx=(85, 55), pady=10)
        ...
        boton_configuraciones = ttk.Button(
            self.medicion_tab, image=configuraciones_icon, command=self.abrir_pestana_configuraciones)
        ...
        columns = ('SKU', 'PackType', 'UOM', 'CNT', 'Largo',
                   'Ancho', 'Alto', 'Peso', 'Fecha')
        self.tree = ttk.Treeview(
            self.medicion_tab, columns=columns, show='headings')
        ...
        # Esto añade bordes alrededor de cada celda
        style.configure("Treeview.Treeview", borderwidth=1)

        # Crear barras de desplazamiento
        y_scroll = ttk.Scrollbar(
            self.medicion_tab, orient="vertical", command=self.tree.yview)
        y_scroll.grid(row=3, column=20, sticky='wns')
        self.tree.configure(yscrollcommand=y_scroll.set)

        ttk.Label(self.medicion_tab, text="Respuesta WebService:").grid(
            row=6, column=0, columnspan=2, padx=10, sticky="w")
        self.response_entry = tk.Text(
            self.medicion_tab, state="disabled", background="#FCFFD0", font=("Arial", 10))
        self.response_entry.config(width=20, height=5)
        self.response_entry.grid(
            row=7, column=0, columnspan=20, pady=5, padx=(10, 10), sticky="nsew")

        self.label_envio_exitoso = ttk.Label(
            self.medicion_tab, text="Envío exitosos: 0", foreground="green")
        self.label_envio_exitoso.grid(
            row=8, column=0, columnspan=2, padx=(120, 0), pady=(0, 10))

        self.label_envio_fallido = ttk.Label(
            self.medicion_tab, text="Envíos fallidos: 0", foreground="red")
        self.label_envio_fallido.grid(
            row=8, column=19, rowspan=2, columnspan=2, padx=(0, 0), sticky="w")
    ...
    def create_configuracion_tab(self):
        ...
        ttk.Label(self.configuracion_tab, text="AUTENTICACIÓN WEBSERVICE", font=("Helvetica", 13)).grid(
            row=0, column=0, columnspan=3, pady=(20, 5), sticky="w", padx=espaciado_lateral)

        ttk.Label(self.configuracion_tab, text="URL del Web Service:").grid(
            row=1, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        url_entry = ttk.Entry(self.configuracion_tab,
                              textvariable=self.api_url,  width=45)
        url_entry.grid(row=1, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="URL Imagen:").grid(
            row=2, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        url_entry = ttk.Entry(self.configuracion_tab,
                              textvariable=self.url_api_image,  width=45)
        url_entry.grid(row=2, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="Client ID:").grid(
            row=3, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        client_id_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.client_id, width=45)
        client_id_entry.grid(row=3, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="Client Secret:").grid(
            row=4, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        client_secret_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.client_secret, width=45)
        client_secret_entry.grid(row=4, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="Usuario:").grid(
            row=5, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        username_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.username, width=45)
        username_entry.grid(row=5, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="Contraseña:").grid(
            row=6, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        password_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.password, width=45)
        password_entry.grid(row=6, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="URL del token:").grid(
            row=7, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        token_url_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.token_url, width=45)
        token_url_entry.grid(row=7, column=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="PROCESAMIENTO DE DATOS", font=("Helvetica", 13)).grid(
            row=8, column=1, columnspan=3, pady=(20, 5), sticky="w", padx=espaciado_lateral)
        ttk.Label(self.configuracion_tab, text="Carpeta Origen Data:").grid(
            row=9, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        ...
        def seleccionar_carpeta():
            ...
            if selected_folder:
                ...
                # Insertar la nueva ruta seleccionada
                carpeta_origen_entry.insert(0, selected_folder)

        folder_label = ttk.Button(
            self.configuracion_tab, image=folder_icon, command=seleccionar_carpeta)
        folder_label.image = folder_icon
        folder_label.grid(row=9, column=4, padx=(5, 0), pady=5, sticky="w")

        carpeta_origen_entry = ttk.Entry(
            self.configuracion_tab, textvariable=self.carpeta_archivos, width=40)
        carpeta_origen_entry.grid(
            row=9, column=2, columnspan=2, pady=5, sticky="w")

        ttk.Label(self.configuracion_tab, text="Carpeta Origen Imagen:").grid(
            row=10, column=1, pady=5, sticky="w", padx=espaciado_lateral)
        ...
        def seleccionar_carpeta_imagen():
            ...
            if selected_folder:
                # Borrar el contenido actual del Entry
                carpeta_origen_imagen_entry.delete(0, tk.END)
                # Insertar la nueva ruta seleccionada
                carpeta_origen_imagen_entry.insert(0, selected_folder)

        folder_label_imagen = ttk.Button(
            self.configuracion_tab, image=folder_icon, command=seleccionar_carpeta_imagen)
        folder_label_imagen.image = folder_icon
        folder_label_imagen.grid(
            row=10, column=4, padx=(5, 0), pady=5, sticky="w")

        carpeta_origen_imagen_entry = ttk.Entry(
            self.configuracion_tab, width=40, textvariable=self.carpeta_imagenes)
        carpeta_origen_imagen_entry.grid(
            row=10, column=2, columnspan=2, pady=5, sticky="w")

        save_image = customtkinter.CTkImage(Image.open(
            "Icons/save.png").resize((100, 100), Image.Resampling.LANCZOS))
        boton_save = customtkinter.CTkButton(self.configuracion_tab, text="Guardar Configuración", border_color="#AFACAC", border_width=1,   corner_radius=5, font=(
            "Helvetica", 14), text_color="#000000", fg_color="#FFFFFF", hover_color="#D9F3FF", width=120, height=20, compound="left", image=save_image, command=self.guardar_configuracion)
        boton_save.grid(row=11, column=2, padx=(10, 30), pady=10)

        cambiar_contraseña_button = ttk.Button(
            self.configuracion_tab, text="Cambiar Contraseña", command=self.abrir_ventana_cambio_contraseña)
        cambiar_contraseña_button.grid(
            row=12, rowspan=3, columnspan=2, padx=10, pady=25, sticky="s")
    ...
    def enviar_data(self, data, url, archivo, es_imagen=False):
        ...
        try:
            ...
            token_response = requests.post(
                self.token_url.get(),
                auth=HTTPBasicAuth(self.client_id.get(),
                                   self.client_secret.get()),
                data={
                    "grant_type": "password",
            ...
            if token_response.status_code == 200:
                    ...
                    if self.response.status_code == 200:
                        try:
                            ...
                            try:
                                ...
                                messagebox.showerror(
                                    "La respuesta no es ni JSON ni XML válido. Contenido de la respuesta:", self.response.text)
                    else:
                        # print("Error en la solicitud:", self.response.text)
                        # Mover el archivo a la carpeta de errores
                        if es_imagen:
                            self.mover_a_carpeta_errores(
                ...
                self.error = True
                messagebox.showerror(
                    f"Error al obtener el token de acceso:", token_response.text)
                # Mover el archivo a la carpeta de errores
                if es_imagen:
                    self.mover_a_carpeta_errores(archivo, es_imagen=True)
                else:
            ...
            self.error = True
            messagebox.showerror(
                "Error de conexión", "No se pudo establecer conexión con el servidor.")
            # Mover el archivo a la carpeta de errores
            if es_imagen:
                self.mover_a_carpeta_errores(archivo, es_imagen=True)
            else:
...
# Procesamiento de archivos e imagenes
    def mover_a_carpeta_errores(self, archivo, es_imagen=False):
        ...
        archivo_con_error = os.path.join(
            carpeta_errores, os.path.basename(archivo))
    ...
    def procesar_archivo(self, archivo):
        ...
        try:
            carpeta_procesados_data = os.path.join(
                self.carpeta_procesados_data)
            if not os.path.exists(carpeta_procesados_data):
                os.makedirs(carpeta_procesados_data)

            nuevo_nombre = os.path.join(
                carpeta_procesados_data, os.path.basename(archivo))
            ...
            with open(archivo, "r") as f:
                ...
                if Packtype == "Unidad-UOM3":
                    ...
                    self.item_id = self.tree.insert('', 'end', values=(
                        SKU, Packtype, Tipodepaquete, Cantidad, Largo, Ancho, Alto, Peso, fecha))
                    self.enviar_data(data, self.api_url.get(),
                                     archivo, es_imagen=False)
                ...
                if not self.error:
                    ...
                    nuevo_nombre = os.path.join(
                        self.carpeta_procesados_data, os.path.basename(archivo))
            ...
            messagebox.showerror(
                "Error", f"Error al procesar el archivo {archivo}: {str(ve)}")

            carpeta_procesados_data_e = os.path.join(
                self.carpeta_procesados_data_e)
            ...
            nuevo_nombre_error = os.path.join(
                carpeta_procesados_data_e, os.path.basename(archivo))
            f.close()
    ...
    def procesar_imagen(self, ruta_imagen):
        try:
            ...
            nuevo_nombre = os.path.join(
                carpeta_procesados_img, os.path.basename(ruta_imagen))
            ...
            with open(ruta_imagen, "rb") as img_file:
                    ...
                    "item": {
                        "attrs": {
                            "attr": [
                                ...
                                {"name": "sku", "value": os.path.basename(ruta_imagen).split(
                                    '_')[0]},  # Obtener el "CODIGO" del nombre de la imagen
                                {"name": "uom", "value": os.path.basename(ruta_imagen).split(
                                    '_')[1].split('.')[0]}  # Obtener el "UN" del nombre de la imagen
                            ]
                        },
                        "resrs": {
                            "res": [
                ...
                img_file.close()
                self.enviar_data(
                    json_imagen, self.url_api_image.get(), ruta_imagen, es_imagen=True)
                self.actualizar_log(SKU, es_imagen=True)

                carpeta_procesados_img = os.path.join(
                    self.carpeta_procesados_img)
                if not os.path.exists(carpeta_procesados_img):
                    os.makedirs(carpeta_procesados_img)
            ...
            messagebox.showerror(
                "Error", f"Error al procesar la imagen:", f"Error: {str(e)}")

    def obtener_archivo_mas_antiguo(self, carpeta, extension=None, es_imagen=False):
        time.sleep(1)
        archivos = [f for f in os.listdir(carpeta) if f.endswith(
            extension)] if extension else os.listdir(carpeta)
        if not archivos:
            return None
        try:
            for archivo in archivos:
                if extension == ".jpg" and es_imagen:
                    datetime.strptime(archivo.split(
                        "_")[2].replace('.jpg', ''), "%Y%m%d%H%M%S")
                elif extension == ".txt" and not es_imagen:
                    datetime.strptime(archivo.split(
                        "_")[1].replace('.txt', ''), "%Y%m%d%H%M%S")
        except Exception as e:
            # Manejar el error individualmente para cada archivo
            messagebox.showerror(
                "Error", f"El archivo ({archivo}): no cumple con la estructura definida {str(e)}")
            carpeta_errores = self.carpeta_procesados_data_e if not es_imagen else self.carpeta_procesados_img_e
            carpeta_errores = os.path.join(carpeta_errores)
        ...
        archivos.sort(key=lambda x: datetime.strptime(x.split("_")[2].replace('.jpg', ''), "%Y%m%d%H%M%S")) if extension == ".jpg" and es_imagen else archivos.sort(
            key=lambda x: datetime.strptime(x.split("_")[1].replace('.txt', ''), "%Y%m%d%H%M%S"))
    ...
    def procesar_archivos_continuamente(self):
        while self.ejecutar:
            try:
                archivo_txt = self.obtener_archivo_mas_antiguo(
                    self.carpeta_archivos.get(), ".txt", es_imagen=False)
                archivo_img = self.obtener_archivo_mas_antiguo(
                    self.carpeta_imagenes.get(), ".jpg", es_imagen=True)  # Ajustar la extensión
...
# Actualización de interfaz
    def update_contadores(self):
        self.label_envio_exitoso.config(
            text=f"Envíos exitosos: {self.envio_exitoso}")
        self.label_envio_fallido.config(
            text=f"Envíos fallidos: {self.envio_fallido}")
    ...
    def actualizar_log(self, SKU, es_imagen=False):
        ...
        if self.error and es_imagen:
            # Habilita la edición temporalmente
            self.response_entry.config(state=tk.NORMAL)
            self.response_entry.insert(
                tk.END, f"{fecha}  SKU={SKU}, Respuesta WS:  {self.response.text}\n", 'warning')
            # Deshabilita la edición temporalmente
            ...
            self.response_entry.config(state=tk.NORMAL)
            self.response_entry.insert(
                tk.END, f"{fecha}  SKU={SKU}, Respuesta WS:  Los datos fueron enviados exitosamente\n", 'ok')
            # Deshabilita la edición temporalmente
            self.response_entry.config(state=tk.DISABLED)
        else:
            # Habilita la edición temporalmente
            self.response_entry.config(state=tk.NORMAL)
            self.response_entry.insert(
                tk.END, f"{fecha}  SKU={SKU}, Respuesta WS:  La imagen fue enviada exitosamente\n", 'image')
            # Deshabilita la edición temporalmente
            self.response_entry.config(state=tk.DISABLED)
        self.response_entry.see(tk.END)  # Desplaza la vista al final del texto
        # Desplaza la vista hacia el final de la tabla
    ...
    def exportar_excel(self, SKU, Packtype, Tipodepaquete, Cantidad, Largo, Ancho, Alto, Peso, fecha):
        ...
        if ruta_completa.exists():
            ...
            # Encabezados
            encabezados = ["SKU", "Packtype", "Tipodepaquete",
                           "Cantidad", "Largo", "Ancho", "Alto", "Peso", "Fecha"]
            for col_num, encabezado in enumerate(encabezados, 1):
                worksheet.cell(row=1, column=col_num, value=encabezado)

        # Agregar nueva fila
        nueva_fila = [SKU, Packtype, Tipodepaquete,
                      Cantidad, Largo, Ancho, Alto, Peso, fecha]
    ...
    def detener_proceso(self):
        try:
            self.boton_iniciar.configure(
                state="normal", background="lightgrey")
            self.boton_detener.configure(
                state="disabled", background="#FF7474")
            # Detiene el hilo de procesamiento
            self.ejecutar = False
            # messagebox.showinfo("Proceso detenido", "El proceso ha sido detenido exitosamente.")
        except Exception as e:
            messagebox.showerror(
                "Error al detener el proceso", f"Error: {str(e)}")
