...
class SerialInterface:
    def __init__(self, root):
        ...
        self.bg_color_unidadbutton= "white"
        self.bg_color_subcajabutton= "white"
        self.bg_color_cajabutton= "white"
        
        self.border_color_unidadbutton="#D2CC05"
        self.border_color_subcajabutton="white"
        self.border_color_cajabutton="white"
...
#CREACIÓN DE VENTANA DE MEDICIÓN
    def create_medicion_tab(self):
        ...
        ttk.Label(self.medicion_tab, text=self.texto_licencia ,background=self.colorbackground, font=("Arial", 9)).grid(row=7, rowspan=1, column=0, padx=(5,0), pady=(0,47), sticky="sw")
        ...
        self.sku_entry.grid(row=0, column=2, columnspan=10, padx=10, pady=5, sticky="w", ipadx=100)
        
        ttk.Label(self.medicion_tab, text="Largo:").grid(row=1, column=1, padx=10, pady=5, sticky="w")
        self.largo_entry = ttk.Entry(self.medicion_tab, textvariable=self.length_var, width=6)
        self.largo_entry.grid(row=1, column=2, padx=10, pady=5, sticky="w")

        ttk.Label(self.medicion_tab, text="Ancho:").grid(row=1, column=3, padx=10, pady=5, sticky="e")
        self.ancho_entry = ttk.Entry(self.medicion_tab, textvariable=self.width_var, width=6)
        self.ancho_entry.grid(row=1, column=4, padx=10, pady=5, sticky="w")

        ttk.Label(self.medicion_tab, text="Alto:").grid(row=1, column=5, padx=10, pady=5, sticky="e")
        self.alto_entry = ttk.Entry(self.medicion_tab, textvariable=self.height_var, width=6)
        self.alto_entry.grid(row=1, column=6, padx=10, pady=5, sticky="w")

        ttk.Label(self.medicion_tab, text="Peso:").grid(row=1, column=7, padx=10, pady=5, sticky="e")
        self.peso_entry = ttk.Entry(self.medicion_tab, textvariable=self.weight_var, width=6)
        self.peso_entry.grid(row=1, column=8, padx=10, pady=5, sticky="w")


        self.unidad_button = customtkinter.CTkButton(self.medicion_tab, text="Unidad", command=self.condicionalunidad, fg_color=self.bg_color_unidadbutton, text_color="black", border_color=self.border_color_unidadbutton, width=100, hover_color="lightgreen", font=('Arial',15), border_width=3, corner_radius=8)
        self.unidad_button.grid(row=2, rowspan=1, column=2, padx=10, pady=5, sticky="n", columnspan=2)
        
        self.unidad_button = customtkinter.CTkButton(self.medicion_tab, text="Subcaja", command=self.condicionalunidad, fg_color=self.bg_color_unidadbutton, text_color="black", border_color=self.border_color_subcajabutton, width=100, hover_color="lightgreen", font=('Arial',15), border_width=2, corner_radius=8, state=DISABLED)
        self.unidad_button.grid(row=2, rowspan=1, column=4, padx=10, pady=5, sticky="n", columnspan=2)
        
        self.unidad_button = customtkinter.CTkButton(self.medicion_tab, text="Caja", command=self.condicionalunidad, fg_color=self.bg_color_unidadbutton, text_color="black", border_color=self.border_color_subcajabutton, width=100, hover_color="lightgreen", font=('Arial',15), border_width=2, corner_radius=8, state=DISABLED)
        self.unidad_button.grid(row=2, rowspan=1, column=6, padx=10, pady=5, sticky="n", columnspan=2)

        self.confirm_button = ttk.Button(self.medicion_tab, text="Confirmar medidas",  compound="right", command=self.confirmar)
        self.confirm_button.grid(row=4, rowspan=1, column=3,padx=10, pady=0, sticky="n", columnspan=2)
        
        self.send_button = ttk.Button(self.medicion_tab, text="Terminar y enviar",  compound="right", command=self.send_data)
        self.send_button.grid(row=4, rowspan=1, column=5,padx=10, pady=0, sticky="n", columnspan=2)

        ttk.Label(self.medicion_tab, text="Respuesta:").grid(row=6, column=1, columnspan=2, padx=5, sticky="w")
        ...
        self.response_entry.grid(row=7, column=1, columnspan=20, pady=5, sticky="nsew")
        ...
        self.tree.grid(row=5, column=1, columnspan=20, pady=(10,10))
        ...
        y_scroll.grid(row=5, column=21, sticky='ns')
        ...
        self.paquetes_enviados_label.grid(row=8, column=1, columnspan=2)
        
        self.paquetes_no_enviados_label = tk.Label(self.medicion_tab, text="Envíos fallidos: 0", font=("Verdana", 10), fg='red')
        self.paquetes_no_enviados_label.grid(row=8,column=3, columnspan=10)
        
#CONDICIONALES POR TIPO DE VENTANA
    def condicionalunidad(self):
        self.tipo_medida="Unidad"
        self.bg_color_unidadbutton="green"
        print(self.bg_color_unidadbutton)
        self.create_medicion_tab()

    def condicionalsubcaja(self):
        self.tipo_medida="Subcaja"
        self.unidad_button.configure(state="disable")


    def condicionalcaja(self):
        self.tipo_medida="caja"
        self.unidad_button.configure(state="disable")

        
    def confirmar(self):
        if self.tipo_medida=="Unidad":
            largo_unit=self.length_var.get()
            ancho_unit=self.weight_var.get()
            alto_unit=self.height_var.get()
            peso_unit=self.weight_var.get()
        elif self.tipo_medida=="Subcaja":
            largo_subcaja=self.length_var.get()
            ancho_subcaja=self.weight_var.get()
            alto_subcaja=self.height_var.get()
            peso_subcaja=self.weight_var.get()
        elif self.tipo_medida=="caja":
            largo_caja=self.length_var.get()
            ancho_caja=self.weight_var.get()
            alto_caja=self.height_var.get()
            peso_caja=self.weight_var.get()
